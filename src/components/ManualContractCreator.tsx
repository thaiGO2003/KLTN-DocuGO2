    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-8">
              ...
              >
                ...
                {availableContracts.map(contract => (
                  <option key={contract.id} value={contract.id}>
                    {contract.title} (v{contract.currentVersion})
                  </option>
                ))}
              </select>
              {parentContractId && (
          ...
          {/* Key Highlights */}
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900">6. Thông tin nổi bật</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Loại hợp đồng</label>
                <select
                  value={contractData.extractedInfo.detailedSummary?.keyHighlights?.contractType || 'internal'}
                  onChange={(e) => setContractData({
                    ...contractData,
                    extractedInfo: {
                      ...contractData.extractedInfo,
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  <option value="internal">Hợp đồng nội bộ</option>
                  <option value="commercial">Hợp đồng thương mại</option>
                </select>
              </div>
            </div>
